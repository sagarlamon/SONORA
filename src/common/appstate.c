...
static char library_real_path_if_diff[SONORA_PATH_MAX] = {0};
...
void artists_db_init(void)
{
        ...
        snprintf(filepath, sizeof(filepath), "%s/sonora/%s", SONORA_DATADIR, ARTISTS_DB_FILE);

        if (!exists_file(filepath)) {
                snprintf(filepath, sizeof(filepath), "/usr/share/sonora/%s", ARTISTS_DB_FILE);
