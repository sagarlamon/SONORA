...
typedef struct
{
        char theme_name[SONORA_NAME_MAX];   /**< Theme display name. */
        char theme_author[SONORA_NAME_MAX]; /**< Theme author name. */
...
typedef struct
{
        ...
        char theme_name[SONORA_NAME_MAX];  /**< Theme filename (without extension). */
        char themeAuthor[SONORA_NAME_MAX]; /**< Author name stored from theme file. */

        int lastVolume; /* volume the last time sonora was run, int between 0 and 100. */
        ...
        PixelData sonoraColorRGB;    /**< Primary application accent RGB color. */

        int chromaPreset; /**< Preset index for chroma-based coloring. */
        char chromaPath[SONORA_PATH_MAX];
        char chromaDevice[SONORA_PATH_MAX];
...
typedef struct
{
        ...
        char command_palette_buf[128];
        int command_palette_len;

        bool show_eq_overlay;
        int eq_selected_band;
        int vinyl_frame;
        bool last_pause_state;
        bool last_stop_state;

        ColorPalette visualizer_palettes[10];
...
typedef struct
{
        char path[SONORA_PATH_MAX];
        char theme[SONORA_NAME_MAX];
        char ansiTheme[SONORA_NAME_MAX];
        char colorMode[6];
        char coverEnabled[2];
        char coverAnsi[2];
        char coverStyle[16];
