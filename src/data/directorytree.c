...
int is_valid_entry_name(const char *name)
{
        ...
        size_t len = strnlen(name, SONORA_PATH_MAX + 1);
        if (len > SONORA_NAME_MAX || len > SONORA_PATH_MAX)
                return 0;
                   ...
                   const char *entry_name)
{
        ...
        size_t parentLen = strnlen(parent_path, SONORA_PATH_MAX + 1);
        size_t nameLen = strnlen(entry_name, SONORA_PATH_MAX + 1);

        if (parentLen > SONORA_PATH_MAX || nameLen > SONORA_PATH_MAX) {
...
static int read_directory(const char *path, FileSystemEntry *parent)
{
        ...
        swprintf(pattern, SONORA_PATH_MAX, L"%ls\\*", wpath);
        ...
        do {
                ...
                char utf8_name[SONORA_PATH_MAX];
                ...
                wchar_t child_wpath[SONORA_PATH_MAX];
...
static int read_directory(const char *path, FileSystemEntry *parent)
{
        ...
        for (int i = 0; i < dir_entries; ++i) {
                ...
                if (entry->d_name[0] != '.' &&
                    ...
                    strcmp(entry->d_name, "..") != 0) {
                        char child_path[SONORA_PATH_MAX];
