...
const char favorites_playlist_name[] = "sonora favorites.m3u";
...
void build_playlist_recursive(const char *directory_path,
                              ...
                              PlayList *playlist)
{
        char expanded_path[SONORA_PATH_MAX - SONORA_NAME_MAX - 1];

        expand_path(directory_path,
                    expanded_path,
                    SONORA_PATH_MAX - SONORA_NAME_MAX - 1);
        ...
        wchar_t wexpanded_path[SONORA_PATH_MAX];
        wchar_t pattern[SONORA_PATH_MAX];
...
{
        char expanded_path[SONORA_PATH_MAX - SONORA_NAME_MAX - 1];
        expand_path(directory_path, expanded_path, SONORA_PATH_MAX - SONORA_NAME_MAX - 1);
        ...
        for (int i = 0; i < num_entries && playlist->count < MAX_FILES; i++) {
                ...
                size_t nameLen = strnlen(entry->d_name, SONORA_NAME_MAX);

                if (pathLen + 1 + nameLen >= SONORA_PATH_MAX) {
...
Node *read_m3u_file(const char *filepath, PlayList *playlist)
{
        ...
        char filename[SONORA_PATH_MAX];
        expand_path(filepath, filename, SONORA_PATH_MAX);
...
int make_playlist(PlayList **playlist, int argc, char *argv[], bool exact_search, const char *path)
{
        ...
        expand_path(path, expanded_path, SONORA_PATH_MAX);
        ...
        if (search_type == ReturnAllSongs) {
                ...
                while (token != NULL) {
                        char buf[SONORA_PATH_MAX] = {0};
                        if (strncmp(token, "song", 4) == 0) {
                                memmove(token, token + 4,
                                        strnlen(token + 4, SONORA_PATH_MAX) + 1);
                                search_type = FileOnly;
                        }
                        trim(token, SONORA_PATH_MAX);
                            ...
                            char *m3u_filename, size_t size)
{
        ...
        if (dot == NULL) {
                ...
                if (base_path[strnlen(base_path, SONORA_PATH_MAX) - 1] == '/') {
...
void load_playlist(const char *directory, const char *playlist_name,
                   PlayList **playlist)
{
        char playlist_path[SONORA_PATH_MAX];

        if (!directory || !playlist_name || !playlist)
                return;

        size_t len = strnlen(directory, SONORA_PATH_MAX);
        if (len == 0)
                return;
...
{
        char expanded_path[SONORA_PATH_MAX];
        expand_path(directory, expanded_path, SONORA_PATH_MAX);
...
{
        char m3u_filename[SONORA_PATH_MAX];
        char expanded_path[SONORA_PATH_MAX];
        expand_path(path, expanded_path, SONORA_PATH_MAX);
