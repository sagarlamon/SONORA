...
bool ensure_default_themes(void)
{
        ...
        char themes_path[SONORA_PATH_MAX];
        ...
        char system_themes[SONORA_PATH_MAX - 256];
        snprintf(system_themes, sizeof(system_themes), "%s/themes", get_system_data_dir());
        DIR *dir = opendir(system_themes);
        if (!dir) {
                snprintf(system_themes, sizeof(system_themes), "/usr/local/share/sonora/themes");
                dir = opendir(system_themes);
        }
        if (!dir) {
                snprintf(system_themes, sizeof(system_themes), "/usr/share/sonora/themes");
                dir = opendir(system_themes);
        }
#ifdef _WIN32
        if (!dir) {
                ...
                if (slash) *slash = '\0';
                snprintf(system_themes, sizeof(system_themes), "%s/themes", exe_dir);
                dir = opendir(system_themes);
        }
#endif
        if (!dir) {
                snprintf(system_themes, sizeof(system_themes), "./themes");
                dir = opendir(system_themes);
        }
        ...
        if (!dir) {
                ...
                free(config_path);
                set_error_message("Couldn't copy themes. The directory wasn't found or sonora doesn't have permissions to read from it.");
                quit();
                return false;
        }
        ...
        while ((entry = readdir(dir)) != NULL) {
                ...
                char full_path[SONORA_PATH_MAX];
                ...
                {
                        char src[SONORA_PATH_MAX], dst[SONORA_PATH_MAX], bak[SONORA_PATH_MAX];
