...
{
        static char result[SONORA_PATH_MAX];
...
Lyrics *loadLyricsFromLRC(const char *path,SongData *songdata)
{
        ...
        if (!file){
                ...
                char curFolder[SONORA_PATH_MAX];
                if (snprintf(curFolder, slash - lrcPath + 1, "%s", lrcPath) >= SONORA_PATH_MAX)
                        return nullptr;
