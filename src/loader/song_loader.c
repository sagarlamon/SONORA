                       ...
                       int depth, bool search_sub_dirs)
{
        ...
        char resolved_path[SONORA_PATH_MAX];
        char *result = NULL;
...
{
        gchar *track_id =
            g_strdup_printf("/org/sonora/tracklist/track%d", track_counter);
        track_counter++;
        return track_id;
}
...
void load_meta_data(SongData *songdata)
{
        ...
        generate_temp_file_path(songdata->cover_art_path, SONORA_PATH_MAX, "cover", ".jpg");
        ...
        if (res == -2) {
                ...
                Model *model = get_model();
                char library_expanded[SONORA_PATH_MAX];
                expand_path(model->library->full_path, library_expanded, SONORA_PATH_MAX);
