...
static const gchar *identity = "sonora";
...
void mpris_init(void)
{
        ...
        if (!conn) {
                ...
                k_log("Failed to connect to D-Bus. Either 1) start D-BUS, 2) recompile with USE_DBUS=0 or 3) use dbus-launch sonora 4) run: doas setcap -r /usr/local/bin/sonora");
        ...
        if (!get_gd_bus_connection()) {
                g_dbus_node_info_unref(introspection_data);
                k_log("Failed to connect to D-Bus. Either 1) start D-BUS, 2) recompile with USE_DBUS=0 or 3) use dbus-launch sonora");
                set_error_message("Failed to connect to D-Bus. Either 1) start D-BUS, 2) recompile with USE_DBUS=0 or 3) use dbus-launch sonora 4) run: doas setcap -r /usr/local/bin/sonora");
                quit();
                return;
        }

        const char *app_name = "org.mpris.MediaPlayer2.sonora";
