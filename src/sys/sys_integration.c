...
int is_sonora_process(pid_t pid)
{
        char comm_path[64];
        ...
        if (file != NULL) {
                if (fgets(process_name, sizeof(process_name), file)) {
                        ...
                        // Check if it's sonora (process name might be truncated to
                        // 15 chars)
                        if (strstr(process_name, "sonora") != NULL) {
                                return 1; // It's likely sonora
                        }
                } else {
                        fclose(file);
                }
        ...
        return 0; // Not sonora or couldn't determine
}
...
void delete_pid_file()
{
        ...
        return;
#else
        char pidfile_path[SONORA_PATH_MAX];
        const char *temp_dir = get_temp_dir();

        snprintf(pidfile_path, sizeof(pidfile_path), "%s/sonora_%d.pid", temp_dir,
                 getuid());
...
void restart_sonora(char *argv[])
{
        ...
        if (oldpid > 0) {
                ...
                if (kill(oldpid, 0) == 0) {
                        ...
                        if (kill(oldpid, SIGUSR1) != 0) {
                                k_log("Failed to signal old sonora (pid %d): %s\n",
                                ...
                                if (kill(oldpid, 0) == 0) {
                                        k_log("Old sonora (pid %d) did not exit in time. Forcing termination.\n",
                                                oldpid);
                        ...
                        k_log("No running sonora process found.\n");
                } else {
                        k_log("Error checking old sonora (pid %d): %s\n",
                                oldpid, strerror(errno));
                }
        ...
        // Replace current process with new sonora instance
        execvp("sonora", argv);

        // Only reached if exec fails
        k_log("Failed to restart sonora via execvp: %s\n",
                strerror(errno));
        _exit(1);
#endif
}

// Ensures only a single instance of sonora can run at a time for the current user.
...
#ifdef __ANDROID__
        (void)argv;
        if (is_process_running(pid) && is_sonora_process(pid))
#else
