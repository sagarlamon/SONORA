...
// clang-format off
const char *LOGO[] =  {"  ___  ___  _  _  ___  ___   __   ",
                       " / __)/ _ \\| \\| |/ _ \\| _ \\ /  \\  ",
                       " \\__ \\ (_) | .\\ | (_) |   // /\\ \\ ",
...
void prepare_playlist_string(Node *node, char *buffer, int buffer_size)
{
        ...
        if (strnlen(node->song.file_path, SONORA_PATH_MAX) >= SONORA_PATH_MAX) {
                buffer[0] = '\0';
                return;
        }

        char file_path[SONORA_PATH_MAX];
        ...
        if (depth >= 0) {
                ...
                if (*iter >= ctx.start_lib_iter) {
                        ...
                        if (entry->is_directory) {
                                ...
                                *chosen_name_len = (int)g_utf8_strlen(orig_name, SONORA_PATH_MAX);
                        } else {
...
{
        if (model->is_paused) {
                return SONORA_CHAR_ICON_PAUSE;
        }
        if (model->is_stopped)
                return SONORA_CHAR_ICON_STOP;

        static const char *vinyl_anim[4] = {
                " (  ◎  ) ",
                " ( / ◎ \\ ) ",
                " ( — ◎ — ) ",
                " ( \\ ◎ / ) "
        ...
        Model *m = (Model *)model;
        m->state.ui.vinyl_frame = (m->state.ui.vinyl_frame + 1) % 4;
        return vinyl_anim[m->state.ui.vinyl_frame];
}
...
void now_playing(bool show_artist, const Model *model, k_Rect region, DrawBuffer *buf, DirtyFlags dirty)
{
        ...
        if (title[0] != '\0') {
                char processed[SONORA_PATH_MAX + 1] = {0};
...
ComponentMsg component_side_cover(const Model *model, k_Rect region, DrawBuffer *buf, DirtyFlags dirty)
{
        ...
        while (corrected_width > indent - cover_indent && target_height > 4) {
                target_height--;
                corrected_width = (int)(target_height * aspect);
        }
        if (corrected_width < 10) corrected_width = 10;

        int row = region.row + lroundf((float)region.height / 2.0f - (float)target_height / 2.0f);
        int col = region.col + cover_indent + 3;
        if (row < region.row) row = region.row;

        int corrected_height = corrected_width / 2;

        if (ui->coverEnabled && !ui->hideSideCover) {
                if (model->songdata_ok && songdata && songdata->cover) {
                        if (dirty & DIRTY_SONG) {
                                if (ui->coverAnsi) {
                        ...
                        // Render fallback thumbnail box & disc artwork
                        CellStyle box_style = cell_style_from_theme(ui->theme.logo);
                        int box_h = target_height > 6 ? target_height : 6;
                        int box_w = corrected_width > 12 ? corrected_width : 12;

                        for (int r = 0; r < box_h; r++) {
                                for (int c = 0; c < box_w; c++) {
                                        const char *ch = " ";
                                        if (r == 0 && c == 0) ch = "┌";
                                        else if (r == 0 && c == box_w - 1) ch = "┐";
                                        else if (r == box_h - 1 && c == 0) ch = "└";
                                        else if (r == box_h - 1 && c == box_w - 1) ch = "┘";
                                        else if (r == 0 || r == box_h - 1) ch = "─";
                                        else if (c == 0 || c == box_w - 1) ch = "│";
                                        else if (r == box_h / 2 && c == box_w / 2 - 1) ch = "🎵";
                                        draw_buffer_set_string(buf, row + r, col + c, ch, box_style);
                                }
                        }
                }
        }
...
ComponentMsg component_playback_status(const Model *model, k_Rect region, DrawBuffer *buf, DirtyFlags dirty)
{
        ...
        if (region.width >= ABSOLUTE_MIN_WIDTH) {
                ...
                if (model->is_paused) {
                        state_icon = SONORA_CHAR_ICON_PAUSE " ";
                } else if (model->is_stopped) {
                        state_icon = SONORA_CHAR_ICON_STOP " ";
                } else {
                        state_icon = " ";
                }
...
ComponentMsg component_footer(const Model *model, k_Rect region, DrawBuffer *buf, DirtyFlags dirty)
{
        ...
        if (region.width >= ABSOLUTE_MIN_WIDTH) {
                ...
                if (model->is_paused) {
                        ...
                        state_icon = " " SONORA_CHAR_ICON_PLAY " ";
                }
                snprintf(icons + icons_len, sizeof(icons) - icons_len, "%s", state_icon);
                icons_len = strnlen(icons, sizeof(icons));
#ifndef __ANDROID__
...
ComponentMsg component_playlist_rows(const Model *model, k_Rect region, DrawBuffer *buf, DirtyFlags dirty)
{
        ...
        for (int i = start_iter; i < start_iter + max_rows; i++) {
                ...
                char buffer[SONORA_NAME_MAX + 1];
                char filename[SONORA_NAME_MAX + 1];
                ...
                prepare_playlist_string(node, buffer, SONORA_NAME_MAX);
...
ComponentMsg component_metadata(const Model *model, k_Rect region, DrawBuffer *buf, DirtyFlags dirty)
{
        ...
        if (strnlen(metadata->title, METADATA_MAX_LENGTH) > 0 && region.height >= 1) {
                ...
                char pretty_title[SONORA_PATH_MAX + 1];
                pretty_title[0] = '\0';
                ...
                if (model->title_delay.active && frame <= width) {
                        char display[SONORA_PATH_MAX + 4];
                        char current_text[SONORA_PATH_MAX + 1];
...
ComponentMsg component_progress_bar(const Model *model, k_Rect region, DrawBuffer *buf, DirtyFlags dirty)
{
        ...
        for (int i = 0; i < region.width; i++) {
                ...
                if (i > elapsed_bars) {
                        ...
                        // Filled / Elapsed with dynamic gradient
                        style = filled_style;
                        if (!filled_style.isAnsi && !current_style.isAnsi && elapsed_bars > 1) {
                                float factor = (float)i / (float)elapsed_bars;
                                style.fg = interpolate_pixel_color(filled_style.fg, current_style.fg, factor);
                        }
...
ComponentMsg component_version(const Model *model, k_Rect region, DrawBuffer *buf, DirtyFlags dirty)
{
        ...
        const char *label = _("sonora version: ");
                            ...
                            DirtyFlags dirty)
{
        ...
        draw_buffer_set_string(buf, row, col, _(" SONORA · MADE by SAGAR "), help_style);
...
#define HELP_LINE(fmt, ...)                                                                      \
        ...
        HELP_LINE(_(" · Add Song To 'sonora favorites.m3u': %s (run with 'sonora .')"),
...
#undef HELP_LINE
        ...
        draw_buffer_set_string(buf, row, col + utf8_display_width(_(" Homepage: ")),
                               "https://github.com/sagarlamon/SONORA", link_style);
