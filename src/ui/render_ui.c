...
bool themes_init(int argc, char *argv[])
{
        ...
        if (!themeLoaded) {
                set_error_message("Couldn't load theme. Please re-install sonora or run 'sudo make install' if it was installed manually");
                ui->colorMode = COLOR_MODE_ALBUM_ONE;
        }
...
{
        int i = system("man sonora");

        if (i != 0) {
                printf(_("Run man sonora for help.\n"));
        }
}
...
int print_about_for_version(Model *model)
{
        ...
        ui->color.r = ui->sonoraColorRGB.r;
        ui->color.g = ui->sonoraColorRGB.g;
        ui->color.b = ui->sonoraColorRGB.b;
...
Layout *build_layout_by_name(const char *name)
{
        ...
        if (!layout) {
                set_error_message("Latest layout not found. Please reinstall sonora, or run 'sudo make install' if sonora was installed by running make.");
