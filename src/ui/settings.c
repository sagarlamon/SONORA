...
const char SETTINGS_FILE[] = "sonorarc";
const char STATE_FILE[] = "sonorastaterc";
...
int get_minicontrols_text(char *text, size_t size, MinicontrolMode mode)
{
        ...
        const char *play = (model->is_stopped || model->is_paused)
                               ? SONORA_CHAR_ICON_PLAY
                               : SONORA_CHAR_ICON_PAUSE;
        switch (mode) {
        case MINICONTROLS_NAV:
                return snprintf(
                    text, size,
                    SONORA_CHAR_ICON_PREVIOUS "  %s  " SONORA_CHAR_ICON_NEXT,
                    play);
...
{
        char *filepath = malloc(SONORA_PATH_MAX);
        if (!filepath) {
                perror("malloc");
                quit();
        }

        int written = snprintf(filepath, SONORA_PATH_MAX, "%s/%s", dir, filename);
        if (written < 0 || written >= SONORA_PATH_MAX) {
...
void load_layout_config(void)
{
        ...
        if (!file) {
                set_error_message(
                    ...
                    "Please reinstall sonora, or run 'sudo make install' "
                    "if sonora was installed by running make.");
        ...
        if (version < SONORA_LAYOUT_VERSION) {
...
void settings_init(AppSettings *settings)
{
        ...
        time_t sonorarc_time = get_file_mtime(sonorarc);
        time_t state_time = get_file_mtime(sonorastaterc);

        // Whatever is the newest file should be loaded last
        // So that users can drop in a new settings file in there
        // And it takes precedence over the sonorastaterc (prefs) file.
        if (sonorarc_time <= state_time) {
...
const char *get_default_music_folder(void)
{
        ...
        if (home != NULL) {
                static char music_path[SONORA_PATH_MAX];
...
{
        char expanded_path[SONORA_PATH_MAX];

        if (path[0] != '\0' && path[0] != '\r') {
                if (expand_path(path, expanded_path, SONORA_PATH_MAX) >= 0) {
...
void set_config(AppSettings *settings, UISettings *ui)
{
        ...
        fprintf(file, "# sonora Settings\n\n");
        fprintf(file, "# IMPORTANT: sonora doesn't write to this file, except for when you run sonora path.\n");
        fprintf(file, "# Delete this file if you installed a new version of sonora and you want to see new settings here.\n");
        fprintf(file, "# sonora will then generate the file with all available settings.\n");
        fprintf(file, "# sonora tracks all in-app settings changes in sonorastaterc, which take precedence over sonorarc.\n\n");
        ...
        fprintf(file, "# Theme's go in ~/.config/sonora/themes (on Linux/FreeBSD/Android), \n");
        fprintf(file, "# and ~/Library/Preferences/sonora/themes (on macOS), \n");
        fprintf(file, "theme=%s\n\n", settings->theme);
        ...
        fprintf(file, "# ~/.config/sonora/themes (on Linux/FreeBSD/Android), \n");
        fprintf(file, "# and ~/Library/Preferences/sonora/themes (on macOS).\n\n");
...
{
    static char path[SONORA_PATH_MAX];

    GetModuleFileNameA(NULL, path, sizeof(path));

    char *slash = strrchr(path, '\\');
    if (slash)
        *slash = '\0';              // remove "sonora.exe"

    strcat(path, "\\share\\sonora");
                             ...
                             const char *dst_path)
{
        ...
        char src_path[SONORA_PATH_MAX];

        char system_layouts[SONORA_PATH_MAX];
        snprintf(system_layouts, sizeof(system_layouts), "%s/layouts", get_system_data_dir());
        DIR *dir = opendir(system_layouts);
        if (!dir) {
                snprintf(system_layouts, sizeof(system_layouts), "/usr/local/share/sonora/layouts");
                dir = opendir(system_layouts);
        }
        if (!dir) {
                snprintf(system_layouts, sizeof(system_layouts), "/usr/share/sonora/layouts");
                dir = opendir(system_layouts);
        }
#ifdef _WIN32
        if (!dir) {
                char exe_dir[SONORA_PATH_MAX];
                GetModuleFileNameA(NULL, exe_dir, sizeof(exe_dir));
                char *slash = strrchr(exe_dir, '\\');
                if (!slash) slash = strrchr(exe_dir, '/');
                if (slash) *slash = '\0';
                snprintf(system_layouts, sizeof(system_layouts), "%s/layouts", exe_dir);
                dir = opendir(system_layouts);
        }
#endif
        if (!dir) {
                snprintf(system_layouts, sizeof(system_layouts), "./layouts");
                dir = opendir(system_layouts);
        }
        ...
        if (!dir) {
                ...
                set_error_message("Couldn't copy layouts. Directory wasn't found or sonora doesn't have permissions to read from it.");
                quit();
                return false;
        }
                               ...
                               const char *backup_name)
{
        ...
        char layout_path[SONORA_PATH_MAX];
        char backup_path[SONORA_PATH_MAX];
        ...
        if (stat(layout_path, &st) == -1) {
                ...
                if (fp) {
                        ...
                        if (version == 0 || version < SONORA_LAYOUT_VERSION)
                                regenerate = true;
...
bool ensure_default_layouts(void)
{
        ...
        char layouts_path[SONORA_PATH_MAX];
