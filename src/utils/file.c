...
void get_directory_from_path(const char *path, char *directory)
{
        ...
        size_t len = strnlen(path, SONORA_PATH_MAX);
        ...
        c_strcpy(directory, dir, SONORA_PATH_MAX - 1);
        directory[SONORA_PATH_MAX - 1] = '\0'; // Ensure null termination

        /// Ensure a trailing '/'
        size_t dlen = strnlen(directory, SONORA_PATH_MAX);

        if (dlen > 0 && directory[dlen - 1] != '/' &&
            dlen + 1 < SONORA_PATH_MAX) {
                directory[dlen] = '/';
                directory[dlen + 1] = '\0';
        }
...
{
        char expanded[SONORA_PATH_MAX];

        expand_path(path, expanded, SONORA_PATH_MAX);
           ...
           const char *allowed_extensions, enum SearchType search_type, bool exact_search, int depth)
{
        ...
        while ((entry = readdir(d)) != NULL) {
                ...
                char entry_path[SONORA_PATH_MAX];
                ...
                if (S_ISDIR(path_stat.st_mode)) {
                        ...
                        bool nameMatch = exact_search
                                             ...
                                             : (c_strcasestr(folded_name, low_case_searching, SONORA_PATH_MAX) != NULL);

                        free(folded_name);

                        if (nameMatch && search_type != FileOnly && search_type != SearchPlayList) {
                                c_strcpy(result, entry_path, SONORA_PATH_MAX - 1);
                                result[SONORA_PATH_MAX - 1] = '\0';
                                found = true;
                                break;
                        }
...
int is_in_temp_dir(const char *path)
{
        ...
        static char tmpdir_buf[SONORA_PATH_MAX + 2];

        if (tmp_dir == NULL || strnlen(tmp_dir, SONORA_PATH_MAX) >= SONORA_PATH_MAX)
                tmp_dir = "/tmp";

        size_t len = strlen(tmp_dir);
        c_strcpy(tmpdir_buf, tmp_dir, SONORA_PATH_MAX);
        tmpdir_buf[SONORA_PATH_MAX] = '\0';
...
#ifdef _WIN32
    snprintf(base_dir, sizeof(base_dir), "%s\\sonora", tmp_dir);
    snprintf(user_dir, sizeof(user_dir), "%s\\sonora\\%s", tmp_dir, username);
#else
    snprintf(base_dir, sizeof(base_dir), "%s/sonora", tmp_dir);
    snprintf(user_dir, sizeof(user_dir), "%s/sonora/%s", tmp_dir, username);
#endif
