...
char *string_to_lower(const char *str)
{
        ...
        size_t length = strnlen(str, SONORA_PATH_MAX);
...
char *string_to_upper(const char *str)
{
        ...
        glong length = g_utf8_strlen(str, SONORA_PATH_MAX);
...
void extract_extension(const char *filename, size_t ext_size, char *ext)
{
        ...
        size_t length = strnlen(filename, SONORA_PATH_MAX);
...
{
        size_t length = strnlen(str, SONORA_PATH_MAX);
        size_t suffixLength = strnlen(suffix, SONORA_PATH_MAX);
...
{
        size_t length = strnlen(str, SONORA_PATH_MAX);
        size_t prefixLength = strnlen(prefix, SONORA_PATH_MAX);
...
{
        char *out = malloc(SONORA_PATH_MAX);
        if (!out)
                return NULL;
        ...
        if (base) {
#ifdef _WIN32
                snprintf(out, SONORA_PATH_MAX, "%s\\sonora", base);
#else
                snprintf(out, SONORA_PATH_MAX, "%s/sonora", base);
#endif
        } else if (home) {
#ifdef __APPLE__
                snprintf(out, SONORA_PATH_MAX, "%s/Library/Preferences/sonora", home);
#elif defined(_WIN32)
                snprintf(out, SONORA_PATH_MAX, "%s\\sonora", home);
#else
                snprintf(out, SONORA_PATH_MAX, "%s/.config/sonora", home);
#endif
        } else {
                free(out);
                return NULL;
...
char *get_prefs_path(void)
{
        ...
        if (base) {
                snprintf(out, SONORA_PATH_MAX, "%s", base);
        } else if (home) {
#ifdef __APPLE__
                snprintf(out, SONORA_PATH_MAX, "%s/Library/Application Support", home);
#elif _WIN32
                snprintf(out, SONORA_PATH_MAX, "%s\\AppData\\Local", home);
#else
                snprintf(out, SONORA_PATH_MAX, "%s/.local/state", home);
#endif
        } else {
                free(out);
                return NULL;
...
char *get_file_path(const char *filename)
{
        ...
        size_t configdir_length = strnlen(configdir, SONORA_PATH_MAX);
        size_t filename_length = strnlen(filename, SONORA_PATH_MAX);

        size_t filepath_length = configdir_length + 1 + filename_length + 1;

        if (filepath_length > SONORA_PATH_MAX) {
                free(configdir);
                return NULL;
        }
